...
def fetch_trades_for_market(
    ...
    """Fetch all trades for a single market ticker up to *max_ts*."""
    trades: list[Trade] = []
    ...
    while True:
        ...
        try:
            raw = client.get(f"/markets/{ticker}/trades", params=params)
        except Exception:
...
def fetch_all_trades(
    ...
    def _fetch(ticker: str) -> tuple[str, list[Trade]]:
        return ticker, fetch_trades_for_market(client, ticker, max_ts)
        ...
        for i, future in enumerate(as_completed(futures), 1):
            ...
            try:
                ...
                if i % 100 == 0 or i == len(tickers):
                    logger.info(
                        "Trades progress: %d/%d tickers (total trades: %d)",
                        i,
                        len(tickers),
                        len(all_trades),
                    )
    ...
    logger.info("Total trades fetched: %d across %d tickers", len(all_trades), len(tickers))
    return all_trades
